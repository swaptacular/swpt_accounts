...
def test_get_accounts(client, account):
    r = client.get('/v1/debtors/1/accounts?limit=100')
    assert r.status_code == 200
    assert r.content_type == 'application/json'
    accounts = json.loads(r.data)
    assert 'self' in accounts
    assert accounts['self'].endswith('/v1/debtors/1/accounts?limit=100')
    ...
    r = client.get('/v1/debtors/1/accounts?start_after=1&limit=100')
    assert r.status_code == 200
    assert r.content_type == 'application/json'
...
def test_get_account(client, account):
    r = client.get('/v1/debtors/1/accounts/666')
    assert r.status_code == 404

    r = client.get('/v1/debtors/1/accounts/1')
    assert r.status_code == 200
    assert r.content_type == 'application/json'
    a = json.loads(r.data)
    assert 'self' in a
    assert a['self'].endswith('/v1/debtors/1/accounts/1')
