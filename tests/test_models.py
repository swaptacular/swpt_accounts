...
def test_configure_account():
    ...
    committed_at_ts = current_ts - 2 * one_year
    account = Account(
        debtor_id=D_ID,
        creditor_id=C_ID,
    ...
    i = account.calc_due_interest(1000, committed_at_ts, current_ts)
    assert abs(i - 100) < 1e-12

    i = account.calc_due_interest(-1000, committed_at_ts, current_ts)
    assert abs(i + 100) < 1e-12

    assert account.calc_due_interest(1000, committed_at_ts, committed_at_ts) == 0
    assert account.calc_due_interest(1000, current_ts, current_ts) == 0
    assert account.calc_due_interest(1000, current_ts, committed_at_ts) == 0

    i = account.calc_due_interest(1000, current_ts - timedelta(days=1), current_ts)
    assert abs(i - 0.26098) < 1e-3

    i = account.calc_due_interest(1000, committed_at_ts, committed_at_ts + timedelta(days=1))
    assert abs(i) == 0
