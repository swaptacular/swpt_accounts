...
def test_configure_account(db_session, current_ts):
    ...
    acs = AccountUpdateSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=C_ID
    ).one()
    ...
    assert acs_obj["commit_period"] == 90 * 24 * 60 * 60
    ...
    assert (
        acs_obj["last_transfer_committed_at"]
        == a.last_transfer_committed_at.isoformat()
    )
    ...
    assert (
        len(
            AccountUpdateSignal.query.filter_by(
                debtor_id=D_ID, creditor_id=C_ID
            ).all()
        )
        == 1
...
def test_make_debtor_payment(db_session, current_ts, amount):
    ...
    p.configure_account(
        ...
        negligible_amount=abs(amount),
    )
    p.make_debtor_payment(
        "test", D_ID, C_ID, amount, TRANSFER_NOTE_FORMAT, TRANSFER_NOTE
    )

    root_change = PendingBalanceChangeSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=p.ROOT_CREDITOR_ID
    ).one()
    assert root_change.principal_delta == -amount
    assert (
        len(
            PendingBalanceChangeSignal.query.filter_by(
                debtor_id=D_ID, creditor_id=C_ID
            ).all()
        )
        == 0
    ...
    if amount < 0:
        transfer_number1 += 1
        assert (
            len(AccountTransferSignal.query.filter_by(debtor_id=D_ID).all())
            == 1
        )
        cts1 = AccountTransferSignal.query.filter_by(
            debtor_id=D_ID, creditor_id=C_ID
        ).one()
        ...
        assert (
            len(
                AccountTransferSignal.query.filter_by(
                    debtor_id=D_ID, creditor_id=p.ROOT_CREDITOR_ID
                ).all()
            )
            == 0
    ...
    p.make_debtor_payment(
        "test", D_ID, C_ID, 2 * amount, TRANSFER_NOTE_FORMAT, TRANSFER_NOTE
    )
    _flush_balance_change_signals()
    p.process_pending_balance_changes(D_ID, C_ID)
    cts = AccountTransferSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=C_ID, transfer_number=transfer_number1 + 1
    ).one()
...
def test_make_debtor_zero_payment(db_session, current_ts):
    ...
    assert not PendingBalanceChangeSignal.query.all()
    _flush_balance_change_signals()
    p.process_pending_balance_changes(D_ID, C_ID)
    p.process_pending_balance_changes(D_ID, p.ROOT_CREDITOR_ID)
...
def test_make_debtor_creditor_account_deletion(db_session, current_ts, amount):
    ...
    cts.principal == 0
    changes = PendingBalanceChangeSignal.query.all()
...
def test_make_debtor_interest_payment(db_session, current_ts, amount):
    ...
    assert root_change.principal_delta == -amount
    assert not PendingBalanceChangeSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=C_ID
    ).all()
...
def test_positive_overflow(db_session, current_ts):
    ...
    p.process_pending_balance_changes(D_ID, C_ID)
    assert (
        not p.get_account(D_ID, C_ID).status_flags
        & Account.STATUS_OVERFLOWN_FLAG
    )
    ...
    p.process_pending_balance_changes(D_ID, C_ID)
    assert (
        p.get_account(D_ID, C_ID).status_flags & Account.STATUS_OVERFLOWN_FLAG
    )
...
def test_get_available_amount(db_session, current_ts):
    q = Account.query.filter_by(debtor_id=D_ID, creditor_id=C_ID)
    q_root = Account.query.filter_by(
        debtor_id=D_ID, creditor_id=p.ROOT_CREDITOR_ID
    )
...
def test_delete_account_negative_balance(db_session, current_ts):
    ...
    p.configure_account(
        ...
        1,
        config_flags=Account.CONFIG_SCHEDULED_FOR_DELETION_FLAG,
        negligible_amount=MAX_INT64,
    )

    assert not p.is_reachable_account(D_ID, C_ID)

    # Verify that re-creating the account clears
    # CONFIG_SCHEDULED_FOR_DELETION_FLAG:
    p.configure_account(D_ID, C_ID, current_ts, 2)
    assert not q.one().status_flags & Account.STATUS_DELETED_FLAG
    assert (
        not q.one().config_flags & Account.CONFIG_SCHEDULED_FOR_DELETION_FLAG
    )
...
def test_prepare_transfer_too_many_prepared_transfers(db_session, current_ts):
    ...
    q = Account.query.filter_by(debtor_id=D_ID, creditor_id=C_ID)
    q.update(
        {Account.principal: 100, Account.pending_transfers_count: MAX_INT32}
    )
    p.prepare_transfer(
        coordinator_type="test",
        coordinator_id=1,
...
def test_commit_prepared_transfer(db_session, current_ts):
    ...
    assert (
        len(AccountTransferSignal.query.filter_by(debtor_id=D_ID).all()) == 2
    )
    cts1 = AccountTransferSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=C_ID
    ).one()
    ...
    cts2 = AccountTransferSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=1234
    ).one()
...
def test_commit_issuing_transfer(db_session, current_ts):
    ...
    pt = PreparedTransfer.query.filter_by(
        debtor_id=D_ID, sender_creditor_id=ROOT_CREDITOR_ID
    ).one()
    p.finalize_transfer(
        D_ID, ROOT_CREDITOR_ID, pt.transfer_id, "issuing", 1, 2, 200
    )
...
def test_commit_to_debtor_account(db_session, current_ts):
    ...
    p.finalize_transfer(
        pt.debtor_id, pt.sender_creditor_id, pt.transfer_id, "test", 1, 2, 40
    )
    p.process_finalization_requests(D_ID, C_ID)
    _flush_balance_change_signals()
    p.process_pending_balance_changes(D_ID, p.ROOT_CREDITOR_ID)
    p.process_pending_balance_changes(D_ID, C_ID)
    assert (
        len(AccountTransferSignal.query.filter_by(debtor_id=D_ID).all()) == 1
    )
    assert len(FinalizedTransferSignal.query.all()) == 1
    cts1 = AccountTransferSignal.query.filter_by(
        debtor_id=D_ID, creditor_id=C_ID
