...
def _on_prepare_transfer_signal(
    ...
    **kwargs
    ...
    """Try to secure some amount, to eventually transfer it to another
    account.
    """
    try:
...
_MESSAGE_TYPES = {
    ...
    "PrepareTransfer": (
        ps.PrepareTransferMessageSchema(),
        _on_prepare_transfer_signal,
    ),
    "FinalizeTransfer": (
        ps.FinalizeTransferMessageSchema(),
        _on_finalize_transfer_signal,
    ),
    "PendingBalanceChange": (
        ps.PendingBalanceChangeMessageSchema(),
        _on_pending_balance_change_signal,
    ),
}
