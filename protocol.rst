...
1. If a corresponding `AD record`_ already exists, the timestamp of
   ...
   record) MUST be set to be the value of the messages's ``ts``
   field. Then the client implementation MUST decide whether the same
   or a later `AccountUpdate`_ message has been received
